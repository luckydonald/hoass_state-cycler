...
- Add unit tests for core↔adapters
  ...
  - Please do proper homeassistant integration tests.
    - It should cover at least:
      - Creating a cycler entity with various configurations
      - Cycling through states using all available actions
      - Verifying state restoration after Home Assistant restart
      - Testing timer functionality for automatic cycling
      - Ensuring events are fired correctly with appropriate attributes
      - Testing multiple cycler entities operating independently
      - Validating UI interactions through the frontend components
      - Test the mode for the timeout-on-off functionality (the "cycle" action, with that mode enabled, waiting for the timeout, then the next action turns it off) - and one test to assert the reverse.
      - Test the include_off_state functionality
      - Check that the attributes are properly set and updated
