...
ifeq ($(BACKEND),1)
	...
	# Suppress the deprecation warning about asyncio_default_fixture_loop_scope early
	# by setting PYTEST_ADDOPTS inline for this pytest invocation (avoids exporting
	# and interacting with shell continuation markers).
	@PYTEST_ADDOPTS="$${PYTEST_ADDOPTS:-} -W ignore::pytest.PytestDeprecationWarning" $(PYTEST) $(PYTEST_OPT) $(PYTEST_ARGS) -m "not playwright"
