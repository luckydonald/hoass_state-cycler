...
**State Cycler** — a Home Assistant custom integration with a Python backend and a Vue.js Lovelace card frontend. Generated from [hoass_state-cycler](https://github.com/luckydonald/hoass_state-cycler) via `scripts/init.sh`.
...
### Python backend (`custom_components/state_cycler/`)
...
- `src/main.ts` — two custom elements registered via `customElements.define`: `state-cycler-card` (the card) and `state-cycler-card-editor` (the editor). Both mount a Vue app into their shadow DOM.
- `src/StateCyclerCard.vue` — main card component (`<script setup lang="ts">`)
